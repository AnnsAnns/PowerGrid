...
use powercable::{charger::ChargeAccept, tickgen::{Phase, TickPayload, TICK_AS_SEC}, ChartEntry, Offer, ACK_ACCEPT_BUY_OFFER_TOPIC, CHARGER_ACCEPT, CHARGER_OFFER_AVG_PRICE, CHARGER_OFFER_AVG_DISTANCE, CHARGER_OFFER_AVG_COST, POWER_TRANSFORMER_PRICE_TOPIC};
...
pub async fn start_transformer() {
    ...
    client.subscribe(ACK_ACCEPT_BUY_OFFER_TOPIC, QoS::ExactlyOnce)
        .await
        .unwrap();
    client.subscribe(CHARGER_ACCEPT, QoS::ExactlyOnce)
        .await.unwrap();
        ...
        if let rumqttc::Event::Incoming(rumqttc::Packet::Publish(p)) = notification {
            match p.topic.as_str() {
                powercable::TICK_TOPIC => {
                    ...
                    client.publish(
                        ...
                        ChartEntry::new(
                            ...
                            tick_payload.timestamp - TICK_AS_SEC).to_string(),
                    ).await.unwrap();
                    client.publish(
                ...
                CHARGER_ACCEPT => {
                    ...
                    avg_accepted_charge_offer_price = total_accepted_charge_offers_price / count_accepted_charge_offers;
                    avg_accepted_charge_offer_distance = total_accepted_charge_offer_distance / count_accepted_charge_offers;
                    avg_accepted_charge_offer_cost = total_accepted_charge_offer_cost / count_accepted_charge_offers;
                    debug!("Calculated averages: count: {}, price: {}, distance: {}, cost: {}",
